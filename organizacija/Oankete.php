...
if($poruka){
    ...
    while($anketa = mysqli_fetch_array($ankete)){
        if($anketa['vreme_isticanja'] < date("Y-m-d h:i:s")){
            $arhiva = "<div class='ml-auto text-danger'>Arhivirana</div>";
        } else {
            $arhiva = "<div class='ml-auto text-success'>Aktuelna</div>";
        }
        if($anketa['brisanje'] == 1){
            $brisanje = '<div id="one"><a href="zatraziBrisanje.php?id_anketa='.$anketa['id_anketa'].'"><img style="width:25px" src="../slike/obrisi.png"><div id="two">Zatraži brisanje</div></a></div>';
        } elseif($anketa['brisanje'] == 0){
        ...
        echo "<div class='list-group-item list-group-item-action shadow'><div class='d-flex'>" . $anketa['vreme_postavljanja']." &nbsp; <a href='OanketaVise.php?id=" . $anketa['id_anketa'] . "'>" . $anketa['naslov']."</a>" . $arhiva. " &nbsp;" . $brisanje."</div></div><br/>";
    }
}
?>

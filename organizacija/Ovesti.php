...
?>
    <!-- stilizovanje dugmeta za brisanje -->
    <style>
        #one {
            position:relative;
            display:block;
  ...
  <div class="collapse navbar-collapse" id="collapsibleNavbar">
    <ul class="navbar-nav align-content-center mx-auto">
        <li class="nav-item flex-fill">
            ...
            <li class="nav-item flex-fill">
                <a class="nav-link text-warning" href="dodajVest.php">Dodaj vest</a>
            </li>
            <li class="nav-item flex-fill">
...
if($poruka){
    ...
    while($vest = mysqli_fetch_array($vesti)){
        if($vest['vreme_isticanja'] < date("Y-m-d h:i:s")){
            $arhiva = "<div class='ml-auto text-danger'>Arhivirana</div>";
        } else {
            $arhiva = "<div class='ml-auto text-success'>Aktuelna</div>";
        }
        if($vest['brisanje'] == 1){
            $brisanje = '<div id="one"><a href="zatraziBrisanje.php?id_vest='.$vest['id_vest'].'"><img style="width:25px" src="../slike/obrisi.png"><div id="two">Zatraži brisanje</div></a></div>';
        } elseif($vest['brisanje'] == 0){
        ...
        echo "<div class='list-group-item list-group-item-action shadow'><div class='d-flex'>" . $vest['vreme_postavljanja']." &nbsp; <a href='OvestVise.php?id=" . $vest['id_vest'] . "'>" . $vest['naslov']."</a>" . $arhiva. " &nbsp;" . $brisanje."</div></div><br/>";
    }
}
?>

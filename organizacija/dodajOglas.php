...
if(isset($_POST["naslov"]) && isset($_POST["vreme_postavljanja"]) && isset($_POST["vreme_isticanja"]) && isset($_POST["tekst"]) && isset($_POST["autor"]) && isset($_FILES["fileToUpload"])){
    ...
    $query = "INSERT INTO oglas (naslov, vreme_postavljanja, vreme_isticanja, tekst, autor, organizacija_id_organizacija) VALUES ('$naslov', '$vreme_postavljanja', '$vreme_isticanja', '$tekst', '$autor', '$id_organizacija')";

    if(mysqli_query($link, $query)){
        $id_oglas = "SELECT id_oglas FROM oglas WHERE naslov='$naslov' AND organizacija_id_organizacija = {$_SESSION['id_organizacija']}";
        $id_oglas = mysqli_query($link, $id_oglas);
        $id_oglasRed = mysqli_fetch_array($id_oglas);
        $id_oglas = $id_oglasRed['id_oglas'];
            ...
            <div class="col-sm-6 list-group bg-light">
                <form action="dodajOglas.php" method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control shadow-sm" name="naslov" id="naslov" placeholder="Naslov oglasa..." value="<?php echo $naslov; ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="vreme_postavljanja">Vreme postavljanja:</label>
                    ...
                    <div class="form-group">
                    <img class="form-control shadow-sm" id="thumbnil" src="" alt="Slika oglasa">
                    </div>
...
<button type="button" class="btn shadow btn-info" name="fileToUpload" id="fileToUpload" onclick="$('#myFileInput').trigger('click');">Dodaj sliku</button>
                    ...
                    <button type="submit" class="btn shadow btn-info" style="margin-botom:100px;">Dodaj oglas</button><br/>
                </form>
            </div>
            <div class="col-sm-3 list-group bg-light"></div>
        </div>
